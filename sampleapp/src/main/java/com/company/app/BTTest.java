...
public class BTTest {
    ...
    void run(String testType, Long threadCount, long timerMillis) throws InterruptedException {
        ...
        while(threadCount-- > 0) {
            ReqThread reqThread = new ReqThread(bigTableSimpleTestApi, testType, map);
            reqThreads.add(reqThread);
            reqThread.start();
        }
...
class ReqThread extends Thread {
    ...
    ReqThread(BigTableSimpleTestApi bigTableSimpleTestApi, String testType, Map <UUID, Data> map){
        this.bigTableSimpleTestApi = bigTableSimpleTestApi;
        this.map = map;
        ...
        while(true) {
            ...
            try {
                if (testType.equals("read")) {
                    ...
                    bigTableSimpleTestApi.read();
                    endTime = System.currentTimeMillis();
                } else if (testType.equals("write")) {
                    startTime = System.currentTimeMillis();
                    bigTableSimpleTestApi.write();
                    endTime = System.currentTimeMillis();
                } else {
...
class Data {
    long send;
    long receive;
    Data(long send, long receive){
        this.send=send;
        this.receive=receive;
    }
}

//class BigTableTestingApi {
...
//    }
//
//    private void createTable(String tableId, List<String> colFamilies) {
...
//    }
//
//    private void writeToTable(String tableId, String rowKey) {
//        try {
...
//    }
//
//    private void readTable(String tableId, String rowKey) {
//        try {
